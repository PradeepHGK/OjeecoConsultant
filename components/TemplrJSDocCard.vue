<template>
 <!-- <p class="flex justify-center font-semibold text-2xl py-8">News / Ad messages</p> -->
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 xl:gap-8 pb-12 px-4">
      ...
      <p class="px-4 pt-10">By combining global expertise, a comprehensive service spectrum, strategic partnerships, a client-centric approach, and a commitment to innovation, Ojeeco Consulting Limited positions itself as the go-to partner for businesses seeking unparalleled solutions for global success.</p>
  </template>
  ...
  const gradientStyle = (card) => {
    return `background: linear-gradient(to top, ${card.bgFrom}, rgba(${card.bgFrom}, 0));`;
  };
  </script>
