<template>
  <div class="relative flex items-center justify-between">
    ...
    <nav class="hidden md:flex md:items-center md:space-x-4">
      <ul class="flex rounded-full bg-white/90 pl-[478px] font-semibold text-lg">
        <li v-for="(item, idx) in useSortBy(navs, ['sort_order'])" :key="`mnu_${idx}`">
          <a v-if="!item.is_action_button && !item.children && item.is_active" :href="item.href" class="relative block px-6 py-2 transition hover:text-teal-500 dark:hover:text-teal-400" :class="{ 'text-teal-500 dark:text-teal-400': isActive(item.href) }">
            {{ item.name }}
