<template>
  <NuxtLayout name="landing">
    ...
    <div class="p-4" id="resume" name="resume">
      <div class="relative mt-0 flex items-center gap-x-4 py-6">
      ...
      <h2 class="text-2xl font-spotlight text-gray-600 mt-4">Professional Synopsis</h2>
      ...
      <!-- Core Competencies -->
      <h2 class="text-2xl font-spotlight text-gray-600 mt-4">Core Competencies</h2>
      ...
      <h2 class="text-2xl font-spotlight text-gray-600 mt-4">Technical Competencies</h2>
      <div class="mt-2 grid grid-cols-2 gap-x-2 md:grid-cols-4 md:gap-y-0">
      ...
      <h2 class="text-2xl font-spotlight text-gray-600 mt-4">Certifications</h2>
      <ul class="list-disc text-sm text-zinc-600 dark:text-zinc-400 pl-6 mt-2">
        <li v-for="item in data.certifications" :key="item">{{ item.name }}</li>
      </ul>
      <!-- Notable Accomplishments -->
      <div v-if="data.notableAccomplishments" class="mt-4">
        <h2 class="text-2xl font-spotlight text-gray-600 mt-4">Notable Accomplishments</h2>
        <ul class="list-disc text-sm text-zinc-600 dark:text-zinc-400 pl-6 mt-2">
      ...
      <!-- Work Experience -->
      <div class="mt-4">
        <h2 class="text-2xl font-spotlight text-gray-600 mt-4">Work Experience</h2>
        <div v-for="job in data.work" :key="job.company" class="mt-4 pb-2">
          ...
          <p class="text-xs mt-1 tracking-tight text-zinc-400 dark:text-zinc-100"><Icon name="icon-park-twotone:calendar-dot" class="w-4 h-4 inline-block mr-1" /> {{ $dayjs(job.startDate).format('DD-MMM-YYYY') }} - {{ job.endDate ? $dayjs(job.endDate).format('DD-MMM-YYYY') : 'Present' }}
</p>
      ...
      <div v-if="data.volunteer" class="mt-4">
        <h2 class="text-2xl font-spotlight text-gray-600 mt-4">Volunteering</h2>
